...
export const AddProductionModal: React.FC<Props> = ({ isOpen, onClose, mixes }) => {
  ...
  const { loading, error, addProduction } = useProductions();

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const finalBagSize = bagSizeType === 'custom' ? Number(customBagSize) : Number(bagSizeType);
    const success = await addProduction(mixId, finalBagSize, Number(quantity));
    if (success) {
      setMixId('');
      setBagSizeType('30');
      setCustomBagSize('');
